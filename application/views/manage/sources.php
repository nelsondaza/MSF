...
<div class="container content">
	...
	<div class="sub-header"><i class="crosshairs icon"></i> <?= lang($class . '_page_name') ?></div>
	<div class="section">
<?
    ...
    if( $this->authorization->is_permitted('create_sources') )
        $table['options'] = anchor( $scope . '/' . $class . '/save', '<i class="plus icon"></i> ' . lang('website_create'), 'class="ui purple button mini"');

    $table['headers'] = array(
        '#',
        lang('sources_column_logo'),
        lang('sources_column_name'),
        lang('sources_column_base_class'),
        lang('sources_column_description'),
        lang('sources_column_creation'),
    );
    ...
    foreach( $sources as $object ) {
        ...
        $table['rows'][] = array(
            $object['id'],
            showSourcePhoto( $object['logo'], array( 'class' => 'ui mini avatar image' ) ),
            $object['name'] . (
                !$object['active']
                ? ' <span class="ui mini red label floated right">' . lang('sources_inactive') . '</span>'
                : ''
            ),
            $object['base_class'],
            $object['description'],
        ...
        if( $this->authorization->is_permitted('update_sources') )
