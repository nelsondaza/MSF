...
<div class="container content">
	...
	<div class="sub-header"><i class="rocket icon"></i> <?= lang($class . '_page_name') ?></div>
	<div class="section">
<?php
    ...
    if( $this->authorization->is_permitted('create_brands') )
        $table['options'] = anchor( $scope . '/' . $class . '/save', '<i class="plus icon"></i> ' . lang('website_create'), 'class="ui purple button mini"');

    $table['headers'] = array(
        '#',
        lang('brands_column_logo'),
        lang('brands_column_name'),
        lang('brands_column_client_name'),
        lang('brands_column_description'),
        lang('brands_column_creation'),
    );
    ...
    foreach( $brands as $object ) {
        ...
        $table['rows'][] = array(
            ...
            $object['name'] . (
                !$object['active']
                ? ' <span class="ui mini red label floated right">' . lang('brands_inactive') . '</span>'
                : ''
            ),
            showClientPhoto( $object['client_logo'], array( 'class' => 'ui mini avatar image' ) ) . $object['client_name'],
            $object['description'],
            $object['creation']
        );

        if( $this->authorization->is_permitted('update_brands') )
