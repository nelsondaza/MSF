				...
				default:
					...
					if (isset($_FILES['account_picture_upload']) && $_FILES['account_picture_upload']['error'] != 4)
					{
						...
						else
						{
							...
							$this->image_lib->initialize(array('image_library' => 'gd2', 'source_image' => FCPATH.RES_DIR.'/user/profile/'.$picture['file_name'], 'new_image' => FCPATH.RES_DIR.'/user/profile/pic_'.md5('account' . $data['account']->id).$picture['file_ext'], 'maintain_ratio' => FALSE, 'quality' => '80%', 'width' => 100, 'height' => 100));
							...
							{
								$data['account_details']->picture = 'pic_'.md5('account' . $data['account']->id).$picture['file_ext'];
